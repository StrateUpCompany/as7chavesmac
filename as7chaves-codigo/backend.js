// Importar dependências necessárias para o backend
...
// Função para inicializar o backend adaptado para Electron
async function initBackend(ipcMain, dbPath) {
  ...
  // Configurar handlers IPC para comunicação com o frontend
...
// Função para configurar os handlers IPC
function setupIpcHandlers(ipcMain, db) {
  ...
  ipcMain.handle('create-lead', async (event, funnelId, lead) => {
    try {
      ...
      await triggerWebhooks(db, funnelId, 'lead_created', newLead);
  ...
  ipcMain.handle('list-webhooks', async () => {
    try {
...
async function triggerWebhooks(db, funnelId, eventType, data) {
  try {
    ...
    const promises = webhooks.map(webhook => triggerWebhook(webhook, data));
